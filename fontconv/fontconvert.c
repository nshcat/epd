...
int main(int argc, char *argv[]) {
  ...
  int numExtraChars = 0;
  unsigned char* extraChars = NULL;

  if(argc > 5) // Extra chars
  {
    numExtraChars = argc - 5;

    extraChars = (unsigned char*)malloc(numExtraChars);

    printf("/* Extra chars:\n");
    for(int i = 0; i < numExtraChars; ++i)
    {
      extraChars[i] = atoi(argv[5 + i]);

      printf("  At 0x%02X: 0x%02X\n", (unsigned char)(last + i + 1), extraChars[i]);
    }
    printf("*/\n\n");
  }
  ...
  if ((!(fontName = malloc(strlen(ptr) + 20))) ||
      (!(table = (GFXglyph *)malloc(((last - first + 1) + numExtraChars ) * sizeof(GFXglyph))))) {
    fprintf(stderr, "Malloc error\n");
    return 1;
  }

  unsigned char actualLast = last;
  if(numExtraChars > 0)
  {
    actualLast = last + numExtraChars;
  }
  ...
  sprintf(ptr, "%dpt%db", size, (actualLast > 127) ? 8 : 7);
  ...
  for (i = first, j = 0; i <= last + numExtraChars; i++, j++) {
    int charToLoad = i;

    if(i > last)
    {
      charToLoad = extraChars[(i-last)-1];
    }

    // MONO renderer provides clean image with perfect crop
    // (no wasted pixels) via bitmap struct.
    if ((err = FT_Load_Char(face, charToLoad, FT_LOAD_TARGET_MONO))) {
  ...
  for (i = first, j = 0; i <= last + numExtraChars; i++, j++) {
    ...
    else if(i < last + numExtraChars)
    {
      unsigned char actualChar = extraChars[(i - last)-1];
      printf(",   // 0x%02X (0x%02X)", (unsigned char)i, actualChar);
      putchar('\n');
    }
  }
  ...
  if(numExtraChars > 0)
  {
    unsigned char lastExtraChar = extraChars[numExtraChars-1];

    printf(" }; // 0x%02X (0x%02X)", last + numExtraChars, lastExtraChar);
    printf("\n\n");
  }
  else
  {
  ...
  if (face->size->metrics.height == 0) {
    ...
    printf("  0x%02X, 0x%02X, %d };\n\n", first, actualLast, table[0].height);
  } else {
